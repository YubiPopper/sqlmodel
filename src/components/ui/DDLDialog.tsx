...
const generateDDL = (table: PhysicalTable): string => {
  ...
  const fkAttributes = table.attributes.filter(a => a.isForeignKey && a.referencesTableId);
  if (fkAttributes.length > 0) {
    lines.push('');
    fkAttributes.forEach((attr) => {
      // Get the referenced table name
      const referencedTable = useModelStore.getState().tables.find(t => t.id === attr.referencesTableId);
      const referencedAttr = referencedTable?.attributes.find(a => a.id === attr.referencesAttributeId);
      
      if (referencedTable && referencedAttr) {
        lines.push(`ALTER TABLE ${table.name}`);
        lines.push(`  ADD CONSTRAINT fk_${table.name}_${attr.name}`);
        lines.push(`  FOREIGN KEY (${attr.name})`);
